...
def _blocked_report(
    ...
    return ProvenanceAuditReport(
        ...
        lifecycle_input_qa={},
        warning=(
            "Phase 5B9 only: audit export was blocked because manifest "
            "validation failed. This is not a result report or strategy conclusion."
        ),
    )
...
def generate_provenance_audit_report(
    ...
    *,
    ...
    if strict and validation["validation_status"] != "PASS":
        return _blocked_report(manifest, validation)
    ...
    return ProvenanceAuditReport(
        ...
        price_snapshot={
            ...
            "validation_status": (
                "PASS" if validation["price_snapshot_rows_hash"] else "MISSING"
            ),
        },
        lifecycle_input_snapshot={
            ...
            "validation_status": (
                "PASS"
                if validation["lifecycle_input_snapshot_rows_hash"]
                else "MISSING"
            ),
        },
        ...
        warning=(
            "Phase 5B9 only: provenance audit reports expose reproducibility, "
